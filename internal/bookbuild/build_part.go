...
func PartCachePath(cacheDir string, partIndex int) string {
	return filepath.Join(cacheDir, fmt.Sprintf("part-%d-overlaid.pdf", partIndex))
}

func PartMergedPath(cacheDir string, partIndex int) string {
	return filepath.Join(cacheDir, fmt.Sprintf("part-%d-merged.pdf", partIndex))
}

func IsPartCached(cacheDir string, partIndex int) bool {
	cachePath := PartCachePath(cacheDir, partIndex)
...
func BuildPart(opts PartBuildOptions) (*PartBuildResult, error) {
	...
	cachePath := PartCachePath(opts.CacheDir, opts.PartIndex)
	mergedPath := PartMergedPath(opts.CacheDir, opts.PartIndex)
...
func LoadCachedPart(cacheDir string, partIndex int) (*PartBuildResult, error) {
	cachePath := PartCachePath(cacheDir, partIndex)

	if _, err := os.Stat(cachePath); err != nil {
		return nil, fmt.Errorf("cached part %d not found: %w", partIndex, err)
	}
...
func ClearPartCache(cacheDir string, partIndex int) error {
	cachePath := PartCachePath(cacheDir, partIndex)
	mergedPath := PartMergedPath(cacheDir, partIndex)
