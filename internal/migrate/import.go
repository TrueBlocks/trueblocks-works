...
func (i *Importer) ReimportAll() error {
	// Disable foreign keys during reimport to avoid constraint violations
	// when INSERT OR REPLACE triggers DELETE on referenced rows
	if _, err := i.db.Conn().Exec("PRAGMA foreign_keys = OFF"); err != nil {
		return fmt.Errorf("disable foreign keys: %w", err)
	}
	defer func() { _, _ = i.db.Conn().Exec("PRAGMA foreign_keys = ON") }()

	if err := i.ReimportCollections(); err != nil {
		return fmt.Errorf("reimport collections: %w", err)
	}
	if err := i.ReimportWorks(); err != nil {
		return fmt.Errorf("reimport works: %w", err)
	}
	if err := i.ReimportOrganizations(); err != nil {
		return fmt.Errorf("reimport organizations: %w", err)
	}
	if err := i.ReimportSubmissions(); err != nil {
		return fmt.Errorf("reimport submissions: %w", err)
	}
...
func (i *Importer) ReimportCollections() error {
...
func (i *Importer) ReimportWorks() error {
...
func (i *Importer) ReimportOrganizations() error {
...
func (i *Importer) ReimportSubmissions() error {
...
func (i *Importer) ReimportCollectionDetails() error {
